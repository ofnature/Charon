...
- **Scans bags *and* armoury** — dungeon and [SealBreaker](https://github.com/ofnature/SealBreaker) loot lands in your main inventory, so that's where it looks first (armoury-only is an opt-in checkbox). Filters by job, equip level, and slot; handles the ring pair and unique-equip rings, and skips the offhand when you're wielding a two-hander.
