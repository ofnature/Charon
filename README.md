...
- **Flex leash in combat:** a tight follow drags a melee toon off its target — it spends the fight running instead of attacking. In ordinary combat the follower holds position until you get past the leash (15y by default), then closes right back up. Adjustable, and boss fights are unaffected since those hand movement to your boss AI entirely.
- **Clean boss handoff:** follow keeps going into the arena and through pre-pull, then hard-stops the instant a boss fight engages (in combat **and** a boss module loaded) so the boss AI owns movement — and resumes automatically when the fight ends. Works with **Minerva** or **BossMod Reborn**, and with Minerva it also holds for a mechanic that punishes movement outright, not just for the fight in general. Normal trash combat never breaks the convoy.
...
- **Choose the venture, and see what it pays** — per retainer: *best it can do*, *pick one*, or *from the farm list*. The catalogue is the game's own sheets (1,114 ventures: what each returns, its five-step quantity tier, the idleness-free gates, exp), ranked by **gil per hour at live market value** with experience as the tiebreak — so it never picks by level. A retainer is only sent out by a button.
- **Retainer contents, readable by other plugins** — what each retainer holds, as last seen, over IPC (`Charon.Retainers.GetContentsJson` / `GetItemJson`): HQ counted separately, every answer carrying the time it was captured, and retainers nobody has opened reported as *unknown* rather than empty. Requests to fetch items back out (`RequestFetch`) and to refresh the store (`RequestRefresh`) are provided, and are off until switched on.
...
- **Doman Donate** lives here too — the Enclave's donation basket pays a gratuity (vendor value × rate) up to a weekly budget that varies by reconstruction stage, and **anything over the budget is eaten**. Charon reads the live budget and rate, splits exactly enough to meet it by the smallest possible margin, stages the stack, presses Donate and answers the confirmation. It refuses to stage any stack larger than the target — that rail exists because a stale number once donated a 999-pile for a fraction of its value. Opening the basket brings up a companion window on the spot: the week's budget and rate, the two steps with the one that is next marked, and the exact stack the game would take — so the flow reads as two steps instead of two buttons.
...
- **Stay logged in when idle** — watches the client's *own* idle timer and sends it one keystroke before it logs us out. A keystroke, not a keypress history: it fires only when the client says it has been idle, it works while the window is in the background, and it reports the timer it saw. This is the tweak for a box that is being kept moving by a plugin — see below — since movement is not input to the client.
...
- Boss fights stay the boss AI's job, exactly as with Fleet Follow — Charon adds the pieces it doesn't have.
