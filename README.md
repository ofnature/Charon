...
- **Lists what you're holding but haven't learned**, with a Collect button on each row. Nothing is ever consumed without a click — unless you turn on **Auto-collect**, which learns the safe kinds on its own (out of combat, one every 1.5s) and still never touches the two kinds below that can be worth real gil.
