...
- Heal Watch (`Features/HealWatch/`): a healer toon tops up fleet toons from LAN roster vitals (`hp`/`entityId` fields), INCLUDING out-of-party ones. `HealWatchManager` is pure (thresholds, emergency queue-jump, 2.5s per-target + global cooldowns vs heartbeat staleness); `Services/Game/HealExecutor.cs` job-gates via `HealActionTable` (one basic heal per job, NOT a rotation) and re-checks LIVE object-table HP before every `UseAction(ActionType.Action, id, targetId)`. Stands down while `Daedalus.IsEnabled` reports the rotation active — never fight the rotation for the action queue.

## Build Baseline
- SDK: `Dalamud.NET.Sdk/15.0.0` (same as Daedalus — do not upgrade independently)
- **Warnings: 0. Tests: 94 passing.** Never commit above/below these baselines.
